export function createCombobox(combobox) {
  let comboboxNode = combobox.querySelector('input');
  let buttonNode = combobox.querySelector('button');
  let listboxNode = combobox.querySelector('ul');
  return new ComboboxAutocomplete(comboboxNode, buttonNode, listboxNode);
}

function toLargeKatakana(str) {
...
class ComboboxAutocomplete {
  constructor(comboboxNode, buttonNode, listboxNode) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      this.allOptionContents.push(node.textContent);
  ...
  addOptions(options, clear=false) {
    if (clear) {
      while (this.listboxNode.children.length > 0) {
        this.listboxNode.removeChild(this.listboxNode.children[0]);
      }
      this.allOptions = [];
      this.filteredOptions = [];
    }
    for (let option of options) {
        ...
        li.innerHTML = option;
        this.listboxNode.appendChild(li);
        this.allOptions.push(li);
        this.allOptionContents.push(li.textContent);
        li.addEventListener('click', this.onOptionClick.bind(this));
        li.addEventListener('pointerover', this.onOptionPointerover.bind(this));
        li.addEventListener('pointerout', this.onOptionPointerout.bind(this));
    }
  }

  getLowercaseContent(node) {
    return toLargeKatakana(node.textContent.toLowerCase());
  }
  ...
  filterOptions() {
    ...
    for (var i = 0; i < this.allOptions.length; i++) {
      ...
      if (
        filter.length === 0 ||
        this.getLowercaseContent(option).includes(toLargeKatakana(filter))
      ) {
    ...
    if (this.allOptionContents.includes(filter)) {
            ...
            if (option) {
              ...
              if (
                this.getLowercaseContent(option).indexOf(
                  toLargeKatakana(this.comboboxNode.value.toLowerCase())
                ) === 0
              ) {
                this.option = option;
